...
def load_walls(
    ...
    voxelTable["wallTemperature"] = (
        Ta  # Initial wall temperature is set to air temperature
    )
...
def wall_surface_temperature(
    ...
    if altitude > 0:
        ...
        for unique_wall in unique_walls:
            ...
            temp_sh = wallsh[
                voxelTable.loc[voxelTable.wallId == unique_wall, "ypos"]
                .to_numpy()
                .astype(int)[0],
                voxelTable.loc[voxelTable.wallId == unique_wall, "xpos"]
                .to_numpy()
                .astype(int)[0],
            ]
    ...
    if altitude > 0:
        ...
        F_sh = cylindric_wedge_voxel(
            (90 - altitude) * deg2rad, voxelTable["svfalfa"].to_numpy()
        ...
        F_sh = 2.0 * F_sh - 1.0  # (cylindric_wedge scaled 0-1)
        ...
        wallSun = wallSun / 180.0
        wallSun[wallSun > 1.0] = 2 - (wallSun[wallSun > 1.0])
        wallSun = (
            0.2 + wallSun * 0.6
        )  # Scaling wall shadows to avoid totally shaded and totally sunlit walls

        # Temperature in shade and sun based on mean values for all voxels, i.e. shade = mean of all shaded and sun = mean of all sunlit
        ts_shade = (
            voxelTable.loc[voxelTable["wallShade"] == 0, "wallTemperature"]
            .to_numpy()
            .mean()
        )
        ts_sun = (
            voxelTable.loc[voxelTable["wallShade"] == 1, "wallTemperature"]
            .to_numpy()
            .mean()
        )
        ...
        Lwallsun = (
            SBC
            * voxelTable["wallEmissivity"].to_numpy()
            * ((ts_sun + 273.15) ** 4)
            * voxelTable["building_fraction"].to_numpy()
            * (1.0 - F_sh)
        ) * wallSun  # (1 - svf - 0.5) istället för viktwall?
        Lwallsh = (
            SBC
            * voxelTable["wallEmissivity"].to_numpy()
            * ((ts_shade + 273.15) ** 4)
            ...
            * (1.0 - F_sh)
            ...
            1 - wallSun
        )  # (1 - svf - 0.5) istället för viktwall?
        ...
        Lwallsh += (
            ...
            * voxelTable["building_fraction"].to_numpy()
            * F_sh
        )  # * 0.5 # (1 - svf - 0.5) istället för viktwall?
    ...
    Lveg = (
        ...
        * voxelTable["wallEmissivity"].to_numpy()
        * (Ta + 273.15) ** 4
        * voxelTable["veg_fraction"].to_numpy()
    )  # * 0.5 # (1 - svf - 0.5) istället för viktwall?
    # Received longwave radiation from the sky
    Lsky = (
        SBC * esky * ((Ta + 273.15) ** 4) * voxelTable["SVF_fix"].to_numpy()
    )  # * 0.5 # svf istället för viktwall?
    # Received reflected longwave radiation
    Lrefl = (
        (1.0 - voxelTable["wallEmissivity"].to_numpy())
        * (Ldown_array + Lup_array)
        * voxelTable["building_fraction"].to_numpy()
    )  # * 0.5 # (1 - svf - 0.5) istället för viktwall?
    ...
    if altitude > 0:
        ...
        voxelTable["F_sh"] = 0.0
        voxelTable["wallSun"] = 0.0

    # Angle of incidence (wall aspect vs solar position (altitude and azimuth))
    sun_x = (
        1
        * np.cos(math.radians(altitude))
        * np.cos(math.radians(azimuth))
        * np.cos(deg2rad * voxelTable["wallAspect"].to_numpy())
    )
    sun_y = (
        1
        * np.cos(math.radians(altitude))
        * np.sin(math.radians(azimuth))
        * np.sin(deg2rad * voxelTable["wallAspect"].to_numpy())
    )
    cf = sun_x + sun_y
    cf[cf <= 0] = 0.0
    ...
    # If sun above horizon
    if altitude > 0:
        K_in = (1 - voxelTable["wallAlbedo"].to_numpy()) * (
            K_direct * cf * voxelTable["wallShade"].to_numpy()
    ...
    Ts, dT = surface_temperature_calc(
        voxelTable["thermalEffusivity"].to_numpy(),
        timeStep,
        K_in,
        L_in,
        Ta,
    ...
    voxelTable["LongwaveRadiation"] = (
        voxelTable["wallEmissivity"]
        * SBC
        * ((voxelTable["wallTemperature"] + 273.15) ** 4)
    ) / np.pi
