...
from umep_solweig.functions.SOLWEIGpython.COMFA.radiationfunctionsCOMFA import (
    CNRRabs_Total,
)
from umep_solweig.functions.SOLWEIGpython.COMFA.radiationfunctionsCOMFA import (
    Rad_Total_solweig,
...
t = hour + minu / 60.0  # decimal time of day (up to 24)
if (yyyy % 4) == 0:
    if (yyyy % 100) == 0:
        if (yyyy % 400) == 0:
            leapyear = 1
...
Total_CNRRabs, zenCOMFA, kdCOMFA = CNRRabs_Total(
    alpha,
    L,
    D,
    Lin,
    ...
    emis,
    Aeff,
)  # np.array([lat]) to deal with timeseries?
