...
def ss_calc(build, cdsm, walls, numPixels, feedback):
    ...
    if (
        buildvec.size > 0
    ):  # TODO: What if vegetation is higher that building OR only vegetation?
...
def writeGridLayout(ssVect, heightMethod, vertHeights, nlayer, skew):
    ...
    if (
        ...
        heightMethod == 3
    ):  # vary number of layers based on height variation. Lowest no of nlayers always 3
        nlayer = 3
        if ssVect[:, 0].max() > 40:
            nlayer = 4
        if ssVect[:, 0].max() > 60:
