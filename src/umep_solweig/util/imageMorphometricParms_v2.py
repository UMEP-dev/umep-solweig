...
def imagemorphparam_v2(dsm, dem, scale, mid, dtheta, feedback, imp_point):
    ...
    build = dsm - dem
    build[(build < 3.0)] = (
        0.0  # building should be higher than 2 meter. Changed to 3 meters #784
    )
    ...
    fai = np.zeros((int(360.0 / dtheta), 1))
    zH = np.zeros((int(360.0 / dtheta), 1))
    zHmax = np.zeros((int(360.0 / dtheta), 1))
    zH_sd = np.zeros((int(360.0 / dtheta), 1))
    pai = np.zeros((int(360.0 / dtheta), 1))
    deg = np.zeros((int(360.0 / dtheta), 1))
    test = np.zeros((int(360.0 / dtheta), 1))
    ...
    for angle in np.arange(0, 360, dtheta):
        if imp_point == 1 and feedback is not None:
        ...
        if mid == 1:  # from center point
            ny = a.shape[0]
            imidy = np.floor((ny / 2.0))  # the mid (NtoS) line of the grid
