...
def COMFA_RAD_SPATIAL_TC(
    L, D, Lin, Lup, Kin, Kup, emis, alpha, Aeff, Kd, metdata, location, utc
):
    """
...
def solar_zenith(lat, d, t):
    ...
    cos_z = (np.dot(np.sin(lat * deg2rad), np.sin(dec * deg2rad))) + (
        np.dot(
            np.dot(np.cos(lat * deg2rad), np.cos(dec * deg2rad)),
            np.cos(np.dot(15.0, (t - to)) * deg2rad),
        )
    )
...
def solar_ET(d):
    ...
    ET = (
        np.dot(-104.7, np.sin(f * deg2rad))
        + np.dot(596.2, np.sin(np.dot(2.0, f) * deg2rad))
        + np.dot(4.3, np.sin(np.dot(3.0, f) * deg2rad))
        - np.dot(12.7, np.sin(np.dot(4.0, f) * deg2rad))
        - np.dot(429.3, np.cos(f * deg2rad))
        - np.dot(2.0, np.cos(np.dot(2.0, f) * deg2rad))
        + np.dot(19.3, np.cos(np.dot(3.0, f) * deg2rad))
    ) / 3600.0
...
def CRT_Acyl(L, D):
    ...
    # pi=3.14
    Acyl = np.dot(np.dot(2, np.pi), (D / 2) ** 2) + np.dot(
        np.dot(np.dot(2.0, np.pi), (D / 2)), L
    )
